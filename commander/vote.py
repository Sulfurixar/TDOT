            ...
            for arg in args:
                if skip > 0:
                    ...
                    if arg.lower() not in self.commands:
                        ...
                        if not 'vote' in data.servers[msg.server.id].customData:
                           results.append(['', data.embedder([["**Error:**", "The 'vote' command has not yet been set up for this server. Set a channel for votes output via ``vote channel (channel)``."]], colour=data.embed_error), msg.channel]);
                        else:
                            if not 'channel' in data.servers[msg.server.id].customData['vote']:
                                results.append(['', data.embedder([["**Error:**", "No channel was found in vote command, rerun ``vote channel (channel)``."]], colour=data.embed_error), msg.channel]);
                            else:
                                channel = discord.utils.find(lambda m: m.id == data.servers[msg.server.id].customData['vote']['channel'], msg.server.channels);
                                if channel is None:
                                    result.append(['', data.embedder([["**Error:**", "Couldn't find channel set in config in this server, rerun ``vote channel (channel)``."]], colour=data.embed_error), msg.channel]);
                                else:
                                    s = '';
                                    for word in args[argpos:]:
                                        s += word + " ";
                                    yield from client.send_message(channel, '', embed=data.embedder([["**Vote:**", s + " "], ["**Author:**", msg.author.display_name + ":" + msg.author.id + ":<@" + msg.author.id + ">"]]));
                        argpos += 1;
                        continue;
#############################################################################
                    if arg.lower() == 'help':
                    ...
                    if arg.lower() == 'channel':
                        ...
                        if not (msg.author.id == msg.server.owner.id or msg.author.id == data.id):
                            ...
                            if channel is None:
                                ...
                                data.servers[msg.server.id].customData['vote'] = {'channel': channel.id}; 
                                data.servers[msg.server.id].update(client);
                                results.append(['', data.embedder([["**Voting Channel:**", "Set voting channel for this server as ``" + channel.name + ":" + channel.id + "``."]]), msg.channel]);
                    #################
                    if arg.lower() == 'make':
                        skip = len(args[argpos + 1:]);
                        if not 'vote' in data.servers[msg.server.id].customData:
