                    ...
                    if arg.lower() == 'channel':
                        ...
                        if not (msg.author.id == msg.server.owner.id or msg.author.id == data.id):
                            ...
                            if channel is None:
                                ...
                                data.servers[msg.server.id].customData['staff'] = {'channel': channel.id}; 
                                data.servers[msg.server.id].update(client);
                                results.append(['', data.embedder([["**Suggestion Channel:**", "Set staff applications channel for this server as ``" + channel.name + ":" + channel.id + "``."]]), msg.channel]);
                    #################
                    if arg.lower() == 'apply':
                        skip = len(args[argpos + 1:]);
                        if not 'staff' in data.servers[msg.server.id].customData:
                           results.append(['', data.embedder([["**Error:**", "The staff applications command has not yet been set up for this server. Set a channel for staff applications' output via ``staff channel (channel)``."]], colour=data.embed_error), msg.channel]);
                        else:
                            if not 'channel' in data.servers[msg.server.id].customData['suggestions']:
                                results.append(['', data.embedder([["**Error:**", "No channel was found in staff applications' command, rerun ``suggestion channel (channel)``."]], colour=data.embed_error), msg.channel]);
                            else:
                                channel = discord.utils.find(lambda m: m.id == data.servers[msg.server.id].customData['staff']['channel'], msg.server.channels);
                                if channel is None:
                                    result.append(['', data.embedder([["**Error:**", "Couldn't find channel set in config in this server, rerun ``staff channel (channel)``."]], colour=data.embed_error), msg.channel]);
                                else:
                                    s = '';
                                    for word in args[argpos + 1:]:
                                        s += word + " ";
                                    yield from client.send_message(channel, '', embed=data.embedder([["**Staff Application:**", "" + s + " "], ["**Aplication by:**", msg.author.display_name + ":" + msg.author.id + ":<@" + msg.author.id + ">"]]));
                    ######################################
#############################################################################
                argpos += 1;
